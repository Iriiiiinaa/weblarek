...
- описывает товар, доступный для отображения и покупки в магазине
...
Интерфейс ошибок покупателя:

- описывает ошибки валидации полей покупателя

`type IErrorsBuyer = Partial<Record<keyof IBuyer, string>>`

Поля типа соответствуют полям интерфейса IBuyer, значением каждого поля является тект ошибки валидации

Интерфейс ответа интернет-магазина с товарами:

- описывает объект, который интернет-магазин возвращает при получении каталога товаров
...
- описывает данные, которые отправляются на сервер при оформлении заказа
...
Интерфейс наследует поля IBuyer

Дополнительные поля интерфейса заказа:
`items: string[]` - масиив идентификаторов товаров в заказе
`total: number` - общая стоимость

Интерфейс результата оформления заказа:

- описывает объект, который сервер возвращает после оформления заказа
...
Поля интерфейса:
`id: string` - индентификатор созданного заказа
...
Методы класса:
`setPayment(payment: TPayment): void` - сохранение данных в модели
`setEmail(email: string): void` - сохранение данных в модели
`setPhone(phone: string): void` - сохранение данных в модели
`setAddress(address: string): void` - сохранение данных в модели
`getData(): IBuyer` - получение всех данных покупателя
`clear(): void` - очистка данных покупателя
`validate(): IErrorsBuyer` - валидация данных

#### Слой коммуникации

Класс AppApi:

- выполняет запрос на сервер и получает объект с массивом товаров

Конструктор:  
`constructor(api: IApi) {}` - принимает объект, который используется для выполнения запроса к серверу

Поля класса:
`private api: IApi` - объект для выполнения запросок к api сервера

Методы класса:
`getProducts(): Promise<IProductsResponse>` - получает с сервера объект с массивом товаров через Get-запрос
`orderProducts(order: IOrder): Promise<IOrderResult>` - отправляет данные о покупателе и выбранных товаров через Post-запрос

...
В интернет-магазине есть две основные сущности данных: товар и покупатель
...
Методы класса:
`ssetPayment(payment: TPayment): void` - сохранение данных в модели
...
`validate(): IBuyerErrors` - валидация данных
...
Поля класса:
`api: IApi` - объект для выполнения запросок к api сервера
